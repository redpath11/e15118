...
void
raw::sweeper_crdc::unpack_raw_data(unsigned short* pCrdc)
{  
...
void
raw::sweeper_crdc::unpack(unsigned short* pCrdc)
{
  ...
  if(is_valid(x_corr))
      ...
      for(int i=0; i< IC_CHANNELS; ++i)
	{
	  if(is_valid(cal[i]))
	    {
	      double denom = 0;
	      for(int k=0; k< IC_CORR_ORDER+1; ++k)
...
raw::sweeper::unpack(unsigned short* pS)
{
  ...
  while(nWords >0)
	...
	case SWEEPER_FP_CRDC_PACKET:
	  ID = *pS;
	  if(ID == 0)  crdc1.unpack(pS+1);
	  if(ID == 1)  crdc2.unpack(pS+1);
